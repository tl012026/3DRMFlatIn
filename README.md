...
## Method
Three models are used: YOLO, SAM2, and VGGT-Omega. We first use YOLO to detect boxes labeled as person, and send those boxes to SAM2 as prompts, where pixel-wise human masks are produced. Then we run VGGT-Omega on the same views to get per-view **depth maps** (full image, not cropped to the person). Person masks must be aligned to the VGGT depth size before they are applied.

Flatten is a 2D depth test, not a 3D-to-2D unwrap. On each view we take:
...
If humans are not flattened, then `C / B` should be no less than a threshold. Below the threshold, treat the person as flattened onto the background.
