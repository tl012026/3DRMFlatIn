...
## Method
Three models are used: YOLO, SAM2, and VGGT-Omega. We first use YOLO to detect boxes labeled as person, and send those boxes to SAM2 as prompts, where pixel-wise human masks are produced. Then we run VGGT-Omega on the same views to get per-view depth (and cameras). Person masks must be aligned to the VGGT image size before they are applied.
